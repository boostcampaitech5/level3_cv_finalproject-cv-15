...
class DogSkinDataset(Dataset):
    def __init__(self, is_train=True, transform=None, k_fold_num=0):
        ...
        dog_data_route = "/opt/ml/data/skin/last_dataset"
        dog_data_dir = os.listdir(dog_data_route)
        jsons = []
        imgs = []
        for name in dog_data_dir:
            if name == "무증상" or name == "train" or name == "val":
                continue
            file_list = os.listdir(os.path.join(dog_data_route, name))
            print(len(file_list))
            for file_name in file_list:
                temp = file_name.split(".")
                if temp[-1] == "json":
                    jsons.append(os.path.join(dog_data_route, name, file_name))
                else:
                    imgs.append(os.path.join(dog_data_route, name, file_name))
        jsons.sort()
        imgs.sort()
    ...
    def classDefine(self, fullname):
        class_name = '유증상/' + fullname.split('/')[6] 
